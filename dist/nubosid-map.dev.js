...
}).addTo(map); // Coordenadas de las ciudades de Nicaragua
...
var apiKey_tem = '3c743983734341f81c2486dcce043049'; // Función para obtener la temperatura y agregar marcadores en el mapa

function getTemperatureAndAddMarker(city, coords) {
  fetch("https://api.openweathermap.org/data/2.5/weather?lat=".concat(coords.lat, "&lon=").concat(coords.lon, "&units=metric&appid=").concat(apiKey_tem)).then(function (response) {
    return response.json();
  }).then(function (data) {
    var temperature = data.main.temp;
    var velocidadViento = data.wind.speed;
    var probPrecipitacion = data.weather[0].description; // Traducir el valor de probPrecipitacion a español si es necesario
    ...
    var marker = L.marker([coords.lat, coords.lon]).addTo(map).bindPopup("<b>".concat(city, "</b><br>Temperatura: ").concat(temperature, " \xB0C <br>velocidadViento: ").concat(velocidadViento, " km/h <br>probPrecipitacion: ").concat(probPrecipitacion));
...
legend.onAdd = function (map) {
  ...
  var colors = ["#FFEAA4", "#FEC18F", "#F1B87C", "#fb9f3a", "#ed7953"];
